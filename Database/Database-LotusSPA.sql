...
CustTel varchar(14) not null unique, -- 1197546-6358
...
EmpTel varchar(14) not null unique,
...
StatusPack char(1) not null default 'A', -- A = Active | D = Disable
...
StatusReserve char(1) not null default 'U', -- U = Unfinished | F = Finished 
IsDeleted char(1) not null default 'N', -- N = No | Y = Yes
...
PayCode int references tbpayment(PayCode) on update cascade
);
...
SELECT * FROM tbCategoryProd;   -- Used/Done  
SELECT * FROM tbProduct;        -- Done

SELECT * FROM tbOptionPay;      -- Used/Done  
SELECT * FROM tbPayment;		-- Used/Done
SELECT * FROM tbPackage;		-- Done
SELECT * FROM tbReserve;		-- Done
